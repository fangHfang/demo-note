...
&lt;!&ndash;                支持xls,xlsx文件&ndash;&gt;
    ...
    <!--    名单弹窗-->
    <el-dialog title="快捷添加" width="1000px" :visible.sync="dialogFormVisible" class="dialog1"
               @close="dialogFormVisible = false;textarea = ''">
    ...
    <el-dialog title="手动添加" width="1000px" :visible.sync="dialog2" class="dialog1" @close="dialog2 = false">
      <p class="title2">名单信息: <i style="color: red">共{{yuDingInfo.number}}人(已添加{{data.length}}人)</i>
        <el-button class="fn-right" size="mini" type="warning" @click="addPerson">添加</el-button>
      </p>
...
export default {
  ...
  methods: {
    ...
    submitOrder (type) {
      if (type === 0) {
        this.$router.push({name: 'tourOrderHetong2', query: {current: 1}})
      } else {
        this.$router.push({name: 'tourOrderHetong4', query: {current: 3}})
      }
    },
    // 删除名单
    deletePerson (index) {
