<template>
  <div class="MuDiCityList" v-cloak>
    <div class="city"  v-clickOutside="hideCityDialog">
      <el-input type="text" style="width: 238px" placeholder="出发城市" v-model="name" suffix-icon="el-icon-arrow-down"
...
export default {
  ...
  methods: {
    ...
    getListData () {
      serverHttp.GetAllMuDiDis({Type: 0}).then(res => {
        if (res.code === 0) {
          this.list = res.data.MuDiDis
          this.changeData()
        }
      })
    },
