...
export default {
  ...
  methods: {
    ...
    getListData () {
      serverHttp.GetTrainStationList({}).then(res => {
        let arr = res.data.stations
        let arr1 = []
        for (const key in arr) {
          arr[key].cityCode = arr[key].stationCode.toUpperCase().charAt(0)
          arr[key].zhanName = key
          // arr1.push(arr[key])
          let keys = arr[key].cityCode
          if (!arr1[keys]) {
            arr1[keys] = [arr[key]]
          } else {
            arr1[keys].push(arr[key])
          }
        }
        this.list = arr1
        this.changeData()
